...
VALID_ROLES = {"SUPER_ADMIN", "ADMIN_WL", "AFFILIATE", "CLIENT"}
...
def _sync_user_from_clerk(token: str, db: Session) -> User:
    ...
    user = db.query(User).filter(User.clerk_user_id == identity.user_id).one_or_none()
    if user:
        return user
    ...
    role = (identity.role_hint or "CLIENT").upper()
